...
export default function KoreaMap({ mode, focus, onClearFocus, onRegionAsk, metric = "max", onStats, amount }) {
  ...
  const [pos, setPos] = useState({ coordinates: [127.8, 36.2], zoom: 1 });

  // 시도별 값
  useEffect(() => {
    compareSubsidy(MODEL, { limit: 500 })
      .then((res) => {
        const mx = {}, sum = {}, cnt = {};
  ...
  function clearSel() {
    ...
    setPos({ coordinates: [127.8, 36.2], zoom: 1 });
    ...
    <div className="map-wrap">
      ...
      <div className={"map-inner" + (is3d ? " tilt" : "")}>
        <ComposableMap projection="geoMercator"
                       projectionConfig={{ center: [127.8, 36.2], scale: 4800 }}
                ...
                geographies.map((geo) => {
                  ...
                  return (
                    <Geography key={geo.rsmKey} geography={geo}
                      ...
                      fill={v != null ? colorFor(norm(v)) : "#eef2f6"}
                      stroke="#ffffff" strokeWidth={0.7}
                      style={{
                        default: { outline: "none", opacity: dim ? 0.3 : 1, transition: "opacity .3s" },
