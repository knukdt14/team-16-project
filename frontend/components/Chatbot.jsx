...
const REGION_KEYWORDS = [
  ...
  "강원", "충북", "충남", "전북", "전남", "경북", "경남", "제주", "성남",
  "수원", "용인", "고양", "창원", "청주", "천안", "전주", "포항"
];

export default function Chatbot({ onMapMode, mapMode, onSelectRegion, onSelectModel }) {
  const [messages, setMessages] = useState([]);
  const [text, setText] = useState("");
  const [loading, setLoading] = useState(false);
  const [online, setOnline] = useState(null);
  const ctx = useRef({ region: null, model: null, lastQ: "" });
  ...
  async function callBackend(question, region, model, displayText) {
    ...
    // 질문 프롬프트 내 지역이 감지되면 즉시 지도 하이라이트
    for (const rk of REGION_KEYWORDS) {
      if (question.includes(rk)) {
        onSelectRegion?.(rk);
        break;
      }
    }
    ...
    try {
      ...
      if (data.entities?.region) {
        ctx.current.region = data.entities.region;
        onSelectRegion?.(data.entities.region);
      }
      if (data.entities?.model) {
        ctx.current.model = data.entities.model;
        onSelectModel?.(data.entities.model);
      }
      if (data.subsidy?.시도 || data.subsidy?.시군구) {
        onSelectRegion?.(data.subsidy.시군구 || data.subsidy.시도);
      }
      if (data.subsidy?.모델명) {
        onSelectModel?.(data.subsidy.모델명);
      }
  ...
  function pickNeed(value, type) {
    if (loading) return;
    if (type === "region") {
      ctx.current.region = value;
      onSelectRegion?.(value);
    } else {
      ctx.current.model = value;
      onSelectModel?.(value);
    }
